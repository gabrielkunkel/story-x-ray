...
When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@/Users/gabrielkunkel/WebstormProjects/story-x-ray/.claude/get-shit-done/references/tdd.md` so gate enforcement rules are available during planning.
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @/Users/gabrielkunkel/WebstormProjects/story-x-ray/.claude/get-shit-done/references/tdd.md to all eligible tasks:
