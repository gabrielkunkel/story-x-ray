...
```bash
timestamp=$(node "/Users/gabrielkunkel/WebstormProjects/story-x-ray/.claude/get-shit-done/bin/gsd-tools.cjs" current-timestamp full --raw)
```
...
<step name="commit">
```bash
node "/Users/gabrielkunkel/WebstormProjects/story-x-ray/.claude/get-shit-done/bin/gsd-tools.cjs" commit "wip: [context-name] paused at [X]/[Y]" --files [handoff-path] .planning/HANDOFF.json
```
</step>
