...
must_haves:
  truths:
    - "STEP_EXAMPLES export exists in src/data/steps.ts with entries for all 16 steps"
    - "Each step has exactly 3 examples: 2 from sourced works and 1 original"
    - "StepExample interface is exported with source, isOriginal, and text fields"
    - "Every original example has isOriginal: true and source: '[Original]'"
  artifacts:
    - path: "src/data/steps.ts"
      provides: "STEP_EXAMPLES data and StepExample interface"
      contains: "export const STEP_EXAMPLES"
  key_links:
    - from: "src/data/steps.ts"
      to: "src/components/CardEditor.tsx"
      via: "named export STEP_EXAMPLES"
      pattern: "export const STEP_EXAMPLES"
---

<objective>
Add the StepExample interface and STEP_EXAMPLES data export to src/data/steps.ts. This plan writes all 48 example entries (16 steps × 3 examples each) as static content. No UI changes — data only.

Purpose: The complete authored content that Plan 02 will render. Getting the content right is the primary value of this phase.
Output: Updated src/data/steps.ts with StepExample interface + fully-populated STEP_EXAMPLES record.
</objective>
...
<interfaces>
<!-- Existing exports from src/data/steps.ts that this plan appends to -->

```typescript
// Already in file — do NOT remove or modify:
...
// Pattern to follow for new interface (from CONTEXT.md):
...
<task type="auto">
  <name>Task 1: Add StepExample interface and STEP_EXAMPLES to steps.ts</name>
  <files>src/data/steps.ts</files>
  <read_first>src/data/steps.ts — read the full file before editing so you append correctly after createFreshSteps()</read_first>
  <action>
Append the following code to the END of src/data/steps.ts, after the closing brace of createFreshSteps(). Do not modify any existing code.

Add exactly this (copy verbatim):
...
Ensure the file compiles: run `npx tsc --noEmit` after saving. Fix any TypeScript errors before marking the task complete.
  </action>
  <verify>
    <automated>cd /Users/gabrielkunkel/WebstormProjects/story-x-ray && npx tsc --noEmit 2>&1 | head -20</automated>
  </verify>
  <done>
    - src/data/steps.ts exports StepExample interface and STEP_EXAMPLES
    - STEP_EXAMPLES has entries for steps 1–16
    - Each step has exactly 3 examples
    - Every step has exactly 1 entry with isOriginal: true and source: '[Original]'
...
|----------|-------------|
| Data file → React render | Static authored strings passed to JSX — no user input involved |

## STRIDE Threat Register

| Threat ID | Category | Component | Disposition | Mitigation Plan |
|-----------|----------|-----------|-------------|-----------------|
| T-10-01 | Tampering | src/data/steps.ts | accept | Static data file in source control; no runtime modification path |
| T-10-02 | Information Disclosure | example text | accept | Content is intentionally public; no PII or secrets in authored examples |
</threat_model>
...
2. `grep -c "isOriginal: true" src/data/steps.ts` returns 16 (one per step)
3. `grep -c "source:" src/data/steps.ts` returns at least 48
4. The file still exports STEP_DEFINITIONS, STEP_HINTS, and createFreshSteps unchanged
</verification>

<success_criteria>
- All 16 steps have STEP_EXAMPLES entries
- Each step contains exactly 3 examples
- Every step has exactly one original (isOriginal: true)
- StepExample interface is exported and matches the CONTEXT.md spec
- TypeScript compiles clean
- No existing exports in steps.ts are modified
</success_criteria>

<output>
After completion, create `.planning/phases/10-step-examples/10-01-SUMMARY.md`
</output>
